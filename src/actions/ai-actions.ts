...
- **Existing parent skill check:** Before creating a new top-level skill, check whether the chain's subject is clearly a sub-discipline of a skill the user already has. If so, use the existing skill as the parent with a specialization rather than creating a new top-level skill (e.g., if the user has "Motorcycling" and the chain is about motorcycle maintenance, use skill="Motorcycling" + specialization="Maintenance"). **However, the relationship must be a genuine practitioner relationship — someone actively developing the parent skill would naturally encounter and practice the chain's subject as part of that skill.** Surface-level keyword overlap (both involve "vehicles", both involve "health", both involve "building") is not sufficient. If the chain's subject is an independent domain that a practitioner of the parent skill would not recognize as part of their practice, create a new top-level skill instead.
