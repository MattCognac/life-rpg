...
export async function DailyView() {
  ...
  const activeToday = dailyQuests.filter((q) =>
    isDailyActiveToday(q.dailyCron)
  );
  ...
  const totalCompletedToday = activeToday.filter(
    (q) => q.completions.length > 0
  ).length;
      ...
      {activeToday.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="norse-card p-4 stat-glow">
            ...
            <div className="font-display text-2xl text-gold">{bestStreak}</div>
          ...
          {activeToday.map((quest) => (
            <DailyQuestCard
              ...
              completedToday={isCompletedToday(
                quest.completions[0]?.completedAt ?? null
              )}
            />
          ))}
        </div>
